...
if($num_rows3<>0 and $notif->experiences) {
...
//-----Page  start - Awards
if($num_rows6<>0 and $notif->awards) {
		...
		$pdf->SetWidths(array(50,55,20,60));
		$pdf->Row(array('Awarding Body','Name of Award','Date Awarded','Award Level'));
		
		$pdf->SetFont('Times','B',10);
		$pdf->SetWidths(array(50,55,20,60));
		while ($award = $result6->fetch_object()) {
			$pdf->Row(array($award->award_body,$award->award_name,date("d-m-Y", strtotime($award->award_date)),$award->award_level));
		}
}		
//-----Page  end - Awards

//-----Page  start - Phd
if($num_rows4<>0 and $notif->phds) {
...
if($num_rows5<>0 and $notif->publications) {
