<?php
	...
	if ($_POST['submit']=='Search') {	
		...
		if (($eligible) && ($paid)) {
			...
			notif_number_date='$notif_number_date' and B.appn_eligible='1' and B.appn_payment_status=2";
		} elseif ($eligible) {
				$query = "SELECT * from notifications A RIGHT JOIN applications B on A.notifid=B.appn_notifid 
				JOIN profile C ON B.appn_emailid=C.emailid where 
				notif_number_date='$notif_number_date' and B.appn_eligible='1'";
			} elseif ($paid) {
				$query = "SELECT * from notifications A RIGHT JOIN applications B on A.notifid=B.appn_notifid 
				JOIN profile C ON B.appn_emailid=C.emailid where 
				notif_number_date='$notif_number_date' and B.appn_payment_status=2";
			} else 
		
				$query = "SELECT * from notifications A RIGHT JOIN applications B on A.notifid=B.appn_notifid JOIN profile C 
					ON B.appn_emailid=C.emailid where notif_number_date='$notif_number_date'";
	} else {
...
<body>
    ...
    <select name="notif_number_date" id="notif_number_date" class="custom-select browser-default" width="50%">
    									<option value="" name="notif_number_date">Select Notification Number </option>
								 <?php 	while ($row1 = $result1->fetch_object()) {	?>

                                		<option value="<?php echo $row1->notif_number_date; ?>" name="notif_number_date"> <?php echo $row1->notif_number_date; ?>
                                  </option>
